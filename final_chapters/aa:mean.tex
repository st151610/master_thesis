...
Let $\alpha>d/2$, where $d$ is the dimension of the covariate vector $X$, 
and write $V:=d/\alpha <2$.
For all 
$
(j,N)\in\mathbb{N}^2
$
...
\end{gather*}
uniformly in $N$.
Confer \cite[§2.7]{vaart2013} for definitions.

Weaker assumption:
Consider the function
\begin{gather*}
  f^{\mathcal{P_N}}_{\varepsilon_N}
  \colon
  \mathcal{X}
  \to
  \R
  ...
  \,.
\end{gather*}
There exists a sequence of function classes
$(\mathcal{F}_N)$
...
for each $N$,
and satisfy
$\norm{F_N}_{L^2(\P)}\le\varepsilon_N$
and
\begin{gather*}
  \log
...
\end{gather*}
for some $k<2$.
The first assumption implies the second 
by \cite[Corollary~2.7.4]{vaart2013}
\begin{lemma}
  Let
$(\mathcal{F}_N)$
be a sequence of (measurable) function classes with 
envelope functions
$(F_N)$
such that for a decreasing sequence
$(\varepsilon_N)\subset(0,1]$ with $\varepsilon_N\to 0$ for $N\to\infty$ and some $k\in(0,2)$ it holds
$
\norm{F_N}_{L^2(\P)}
\le\varepsilon_N$
and
