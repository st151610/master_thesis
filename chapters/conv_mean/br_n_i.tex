...
For $z\in\R$ we define the function
  ...
  \begin{gather}
    ...
    \end{split}
  \end{gather}
and provide bracketing numbers for these classes.
\begin{lemma}
  \label{aa:mean:l:br}
...
We now consider the stochastic process (indexed over $x\in\R^d$)
\begin{gather}
  \label{error_process}
  \mathbf{1}{
    \left\{ 
...
We show, 
that under mild regularity conditions on the inverse propensity score function all paths of \eqref{error_process} are contained in shrinking function classes $(\mathcal{F}_N)$ --- and provide bracketing numbers. 
...
We derive sufficient conditions on the regularity of the inverse propensity score function.
\begin{lemma}
  \label{lem:br_n_st}
...
\end{gather*}
such that
for all $N\in\mathbb{N}$ the paths of the stochastic process~\eqref{error_process}
are contained in $\mathcal{F}_N$.
\end{lemma}
\begin{proof}
  We want to employ Lemma~\ref{vdv_coro}. 
  To do this, the crucial observation is that by Lemma~\ref{lem:meas_w_0}.\textit{(i)}
  \begin{gather*}
    \text{the paths}\quad
    w_0^\dagger(\cdot)(\omega)
    \quad
...
\begin{remark}
  ...
  we readily obtain $L^2(\P)$ bracketing numbers. Note that $w_0^\dagger(X)$ and $1/\pi(X)$ are independent of $(T,Y)$.  
\end{remark}
A finite covariate space always meets the requirements of Lemma~\ref{lem:br_n_st} --- and that a continuous distribution of $X$ never does so.
\begin{lemma}
  \label{lem:x_finite}
...
\begin{takeaways}
 Bracketing numbers measure the complexity of a class of functions. We can construct function classes that contain all paths of a stochastic process. By Donsker's Theorem or maximal inequalities we can employ bracketing numbers (via bracketing integral) to derive probabilistic properties of empirical processes indexed over function classes. 
\end{takeaways}
